...
# Previous seeds: 42, 37, 21, 1, 2, 3, 4, 5, 6, 7, 8
np.random.seed(42)
...
def get_random_models():
    ...
    for classes in os.listdir(pwd):
        ...
        for model_name in os.listdir(class_dir):
            ...
            if os.path.isfile(f'{model_dir}/{suffix_dir}'):
                models.append(f'{classes}/{model_name}')
        random_models.append(np.random.choice(models, size=selection_size, replace=False))
