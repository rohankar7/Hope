...
# Weights:
# weights_7: Epoch 500, Loss: 0.0017602771444944665
# First 200 triplanes from the triplane_images_128 dir
...
class VAE(nn.Module):
    def __init__(self):
        ...
        self.encoder_conv = nn.Sequential(
            ...
            nn.Conv2d(num_channels, hidden_dim, kernel_size=4, stride=2, padding=1, bias=False),
            nn.BatchNorm2d(hidden_dim),
            nn.ReLU(),
            # nn.MaxPool2d(1),
            # nn.Dropout(0.2),
            nn.Conv2d(hidden_dim, latent_channels_dim, kernel_size=4, stride=2, padding=1, bias=False),
        ...
        self.decoder_conv = nn.Sequential(
            nn.ConvTranspose2d(latent_channels_dim, hidden_dim, kernel_size=4, stride=2, padding=1),
            nn.BatchNorm2d(hidden_dim),
            nn.LeakyReLU(),
            # nn.Dropout(0.1),
    ...
    def decode(self, z):
        ...
        z = z.view(3, latent_channels_dim, latent_dimension, latent_dimension)
...
def vae_loss(recon_x, x, mu, logvar, lambda_tv=1e-3):
...
def train_vae():
    ...
    scheduler = ReduceLROnPlateau(optimizer, 'min', factor = 0.5, patience=5, cooldown=2)
    num_epochs = 500
    best_loss = torch.inf
