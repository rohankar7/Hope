...
class VAE(nn.Module):
    ...
    def decode(self, z):
        x = self.decoder_fc(z)
        x = x.view(x.size(0), -1).unsqueeze(1).reshape(x.size(0), 3, int(math.sqrt(x.size(1))), int(math.sqrt(x.size(1)))) # Reshape to (batch_size, channels, 4, 4)
