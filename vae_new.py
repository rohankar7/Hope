...
class VAE(nn.Module):
    ...
    def encode(self, x):
        ...
        x = x.view(latent_channels_dim, latent_dimension, latent_dimension)
        return x
...
def lr_scheduler_func(epoch, num_epochs, warmup_epochs=config.vae_warmup_epochs, min_lr=config.vae_lr):
...
def train_vae():
    ...
    for epoch in range(num_epochs):
        ...
        if (epoch + 1) % (config.vae_epochs//2) == 0:
            torch.save(weights_config, f'{config.vae_weights_dir}/vae_new_{epoch+1}_new.pth')

def load_vae_checkpoint():
    vae = VAE().to(device)
    checkpoint = torch.load(f"{config.vae_weights_dir}/vae_new_{config.vae_epochs}.pth")
    optimizer = optim.Adam(vae.parameters(), lr=config.vae_lr, betas=(0.5, 0.999))
...
def save_latent_representation():
    device = 'cpu'
    vae  = VAE().to(device)
    vae_weights_dir = f'{config.vae_weights_dir}/vae_new_{config.vae_epochs}_new.pth'
    # vae_weights_dir = f'{config.vae_weights_dir}/{weights_dir}_{1600}_new.pth'
    ...
    with torch.no_grad():
        for i, triplanes in enumerate(triplane_dataloader()):
            ...
            z_decoded = z_decoded.view(p, c, h, w)
