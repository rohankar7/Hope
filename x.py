...
def project_to_plane(mesh, plane='xy', resolution=128):
...
def viz_projections(xy_projection, yz_projection, zx_projection):
    ...
    plt.imshow(xy_projection, cmap='gray')
    plt.subplot(1, 3, 2)
    plt.title('YZ Projection')
    plt.imshow(yz_projection, cmap='gray')
    plt.subplot(1, 3, 3)
    plt.title('ZX Projection')
    plt.imshow(zx_projection, cmap='gray')
    plt.show() 

def generate_triplanes(file_path, resolution=128):
    ...
    return xy_projection, yz_projection, zx_projection_rotated

def model_to_triplanes(out_dir, resolution):
    resolution = 128  # Define the resolution of the voxel grid
    for path in subclasses_list:
...
def main():
    model_to_triplanes('./images', resolution=128)
